                                  ...
                                  <tr v-for="(personal,index) in listaPersonal"> 
                                      ...
                                      <td style="width:100px; text-align:center">
                                        <div v-if="$auth.can('view-renew-per')">
                                          <button type="button" @click="GenerarRepPersonal(personal.id)" class="btn btn-success btn-sm">
   ...
   computed: {
    ...
    methods: {
      ...
      ListarPersonal(page){
          ...
          GenerarRepPersonal(id_personal){ //DGAE
              // this.$v.$reset();
              // if(!this.$v.$invalid){
              // window.open('http://sipefab.fab.bo/certificadoDestAscenso?id_p='+id_personal);
              window.open('http://127.0.0.1:8000/personal?id_p='+id_personal);
              // }else{
              //     this.$v.$touch();
              //     Swal.fire({
              //         icon: 'warning',
